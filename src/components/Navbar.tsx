        ...
        <div className="hidden md:flex items-center space-x-3">
          {navItems.map((item) => (
            <RollingText
              ...
              to={item.path}
              text={item.name}
              className={`flex items-center justify-center px-6 py-2.5 rounded-full text-[13px] uppercase font-semibold tracking-[0.1em] transition-all duration-300 active:scale-95 outline-none ring-0 ${location.pathname === item.path
          ...
          >
            {navItems.map((item) => (
              <Link
                ...
                className={`px-6 py-4 rounded-2xl font-semibold uppercase tracking-[0.1em] text-[12px] transition-all outline-none ring-0 ${location.pathname === item.path
                    ? 'bg-blue-600/10 text-blue-400 border border-blue-500/20'
